    ...
    (props: IProps): React.ReactElement<IProps> => {
      ...
      const fetchQuestions = (): void => {
        MyAxios(window.location.href)
          ...
          .get<IQuestions>('/DyObservationTwenty')
      ...
      const handleChange = (event: ChangeEvent<HTMLInputElement>): void => {
        ...
        MyAxios(window.location.href)
          ...
          .post<IBasicReturn>('/DyObservationTwenty/ChangeActive',
          {
      ...
      return (
        <Container className={classes.root}>
                ...
                <List>
                  {state.Questions.map((q, i): React.ReactElement => (
                    <React.Fragment key={`Frag-${q.Guid}`}>
                      ...
                      <Question number={(i + 1)} question={q} key={q.Guid} handleChange={handleChange} to="dyObservationTwenty" />
          ...
          <Add to="/dynamicContent/DyObservationTwenty/add" />
        </Container>
      );
    },
  ),
