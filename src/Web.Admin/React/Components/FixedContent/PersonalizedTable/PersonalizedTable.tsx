...
import PersonalizedTable from '../FirstPage/Form/Form';
...
const MyPersonalizedTable = withFormik<WithTranslation & WithSnackbarProps & IProps, IStaticFirstPage>({
  ...
  handleSubmit: async (values, { setSubmitting, props }): Promise<void> => {
    ...
    await myAxios(window.location.href).post<IStaticFirstPage>('StaticContent/PersonalizedTableEdit', {
      ...
      if (data.Success) {
        enqueueSnackbar(t('StaticPersonalizedTable:feedback.success'), {
          variant: 'success',
        });
      } else {
        enqueueSnackbar(t('StaticPersonalizedTable:feedback.failure'), {
          variant: 'error',
        });
      }
    }).catch((): void => {
...
export const Login = withTranslation()(withSnackbar(MyPersonalizedTable));
