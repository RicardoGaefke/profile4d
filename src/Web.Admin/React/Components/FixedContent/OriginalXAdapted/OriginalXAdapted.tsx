...
const MyOriginalXAdapted = withFormik<WithTranslation & WithSnackbarProps & IProps, IStaticFirstPage>({
  ...
  handleSubmit: async (values, { setSubmitting, props }): Promise<void> => {
    ...
    await myAxios(window.location.href).post<IStaticFirstPage>('StaticContent/OriginalXAdaptedEdit', {
      ...
      if (data.Success) {
        enqueueSnackbar(t('StaticOriginalXAdapted:feedback.success'), {
          variant: 'success',
        });
      } else {
        enqueueSnackbar(t('StaticOriginalXAdapted:feedback.failure'), {
          variant: 'error',
        });
      }
    }).catch((): void => {
...
export const Login = withTranslation()(withSnackbar(MyOriginalXAdapted));
