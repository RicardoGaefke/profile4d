...
import DominantResources from '../Introduction/Form/Form';
...
export const MyDominantResources = withFormik<WithTranslation & WithSnackbarProps & IProps, IStaticIntroduction>({
  ...
  handleSubmit: async (values, { setSubmitting, props }): Promise<void> => {
    ...
    await myAxios(window.location.href).post<IStaticIntroduction>('StaticContent/DominantResourcesEdit', {
      ...
      if (data.Success) {
        enqueueSnackbar(t('StaticDominantResources:feedback.success'), {
          variant: 'success',
        });
      } else {
        enqueueSnackbar(t('StaticDominantResources:feedback.failure'), {
          variant: 'error',
        });
      }
    }).catch((): void => {
...
export const Login = withTranslation()(withSnackbar(MyDominantResources));
    ...
    return (
      <div className={classes.main}>
        <Typography
          ...
          variant="h5"
        >
          {t('StaticDominantResources:title')}
        </Typography>
