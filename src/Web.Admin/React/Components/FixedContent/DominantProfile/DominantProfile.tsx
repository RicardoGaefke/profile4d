...
import DominantProfile from '../Introduction/Form/Form';
...
export const MyDominantProfile = withFormik<WithTranslation & WithSnackbarProps & IProps, IStaticIntroduction>({
  ...
  handleSubmit: async (values, { setSubmitting, props }): Promise<void> => {
    ...
    await myAxios(window.location.href).post<IStaticIntroduction>('StaticContent/DominantProfileEdit', {
      ...
      if (data.Success) {
        enqueueSnackbar(t('StaticDominantProfile:feedback.success'), {
          variant: 'success',
        });
      } else {
        enqueueSnackbar(t('StaticDominantProfile:feedback.failure'), {
          variant: 'error',
        });
      }
    }).catch((): void => {
...
export const Login = withTranslation()(withSnackbar(MyDominantProfile));
