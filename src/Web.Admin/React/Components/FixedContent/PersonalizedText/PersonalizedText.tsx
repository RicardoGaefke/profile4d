...
import PersonalizedText from '../FirstPage/Form/Form';
...
const MyPersonalizedText = withFormik<WithTranslation & WithSnackbarProps & IProps, IStaticFirstPage>({
  ...
  handleSubmit: async (values, { setSubmitting, props }): Promise<void> => {
    ...
    await myAxios(window.location.href).post<IStaticFirstPage>('StaticContent/PersonalizedTextEdit', {
      ...
      if (data.Success) {
        enqueueSnackbar(t('StaticPersonalizedText:feedback.success'), {
          variant: 'success',
        });
      } else {
        enqueueSnackbar(t('StaticPersonalizedText:feedback.failure'), {
          variant: 'error',
        });
      }
    }).catch((): void => {
...
export const Login = withTranslation()(withSnackbar(MyPersonalizedText));
