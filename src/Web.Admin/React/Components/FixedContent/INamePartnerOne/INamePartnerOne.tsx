...
import INamePartnerOne from '../FirstPage/Form/Form';
...
export const MyINamePartnerOne = withFormik<WithTranslation & WithSnackbarProps & IProps, IStaticFirstPage>({
  displayName: 'Static Content Internal First Partners Name',
  ...
  handleSubmit: async (values, { setSubmitting, props }): Promise<void> => {
    ...
    await myAxios(window.location.href).post<IStaticFirstPage>('StaticContent/IntNamePartnerOneEdit', {
      ...
      if (data.Success) {
        enqueueSnackbar(t('StaticINamePartnerOne:feedback.success'), {
          variant: 'success',
        });
      } else {
        enqueueSnackbar(t('StaticINamePartnerOne:feedback.failure'), {
          variant: 'error',
        });
      }
    }).catch((): void => {
...
export const Login = withTranslation()(withSnackbar(MyINamePartnerOne));
