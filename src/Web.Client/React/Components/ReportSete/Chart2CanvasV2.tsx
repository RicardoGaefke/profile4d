...
const Chart2CanvasV2 = (props: Chart2CanvasProps): JSX.Element => {
  ...
  const data = {
    labels: [
      `Comandante - ${((comandante / 77) * 100).toFixed(2)}%`,
      `Mediador - ${((mediador / 77) * 100).toFixed(2)}%`,
      `Organizador - ${((organizador / 77) * 100).toFixed(2)}%`,
      `Prestativo - ${((prestativo / 77) * 100).toFixed(2)}%`,
      `Realizador - ${((realizador / 77) * 100).toFixed(2)}%`,
      `Criativo - ${((criativo / 77) * 100).toFixed(2)}%`,
      `Analítico - ${((analitico / 77) * 100).toFixed(2)}%`,
      `Planejador - ${((planejador / 77) * 100).toFixed(2)}%`,
      `Visionário - ${((visionario / 77) * 100).toFixed(2)}%`,
    ],
    datasets: [
      {
        data: [
